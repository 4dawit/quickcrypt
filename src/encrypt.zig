pub fn encrypt(crypt_type: CryptType) !void {
    switch (crypt_type) {
        ...
        .SHA1 => |value| {
            try simpleBytesToHex(std.crypto.hash.Sha1, value);
        },

        .SHA2_256 => |value| {
            try simpleBytesToHex(std.crypto.hash.sha2.Sha256, value);
        },

        .SHA2_512 => |value| {
            try simpleBytesToHex(std.crypto.hash.sha2.Sha512, value);
        },

        .SHA3_256 => |value| {
            try simpleBytesToHex(std.crypto.hash.sha3.Sha3_256, value);
        },

        .Blake2b512 => |value| {
            try simpleBytesToHex(std.crypto.hash.blake2.Blake2b512, value);
        },

        .Blake3 => |value| {
            try simpleBytesToHex(std.crypto.hash.Blake3, value);
        },

        .MD5 => |value| {
            try simpleBytesToHex(std.crypto.hash.Md5, value);
        },
...
fn simpleBytesToHex(hash_type: anytype, value: []const u8) !void {
